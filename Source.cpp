...
GLfloat LightColor[][3] = { 1,1,0,   0,1,1,   0,1,0 };//yellow,
...
void introScreen()
{
	...
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-675, 490, 0.0, "THAPAR UNIVERSITY OF ENGINEERING & TECHNOLOGY");
	...
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-140, 80, 0.0, "Group of 3CO25");
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-800, -100, 0.0, " STUDENT NAMES");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-800, -200, 0.0, " Sushant Vij");
	displayRasterText(-800, -285, 0.0, " Sahil Chhabra");
	displayRasterText(-800, -370, 0.0, " Srimon Ghosh");
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(500, -100, 0.0, "Under the Guidance of");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(500, -200, 0.0, "MISS JOYTI");
	glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-150, -400, 0.0, "Academic Year 2023");
	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-225, -550, 0.0, "Press ENTER to start the game");
	glFlush();
	glutSwapBuffers();
}

void startScreenDisplay()//graphics rendering
{
	glLineWidth(10);
...
//This block of code is checking if the mouse is within a specific range of coordinates on the screen
		glColor3f(0, 0, 1);
		if (mButtonPressed) {
...
void backButton() {
	if (mouseX <= -450 && mouseX >= -500 && mouseY >= -275 && mouseY <= -250) {
		...
		if (mButtonPressed) {
			...
			mButtonPressed = false;
			glutPostRedisplay();
			//the display callback function registered for the current window is called during the next iteration
			//shows real time execution of screen display
		}
	}
...
void displayGameOverMessage() {
	glColor3f(1, 1, 0);
	const char* message;  // Declare message as a const char*
	if (alienLife1 > 0)
